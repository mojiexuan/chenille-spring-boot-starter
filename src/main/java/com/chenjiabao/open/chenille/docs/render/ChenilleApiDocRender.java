...
public class ChenilleApiDocRender {
    ...
    private final ChenilleApiDocsTemplate template = new ChenilleApiDocsTemplate();
    ...
    private String renderNavGroup(String path) {
        return ChenilleApiScanner.getChenilleApiGroups().stream().map(apiGroup -> {
     ...
     * 渲染侧边导航栏选项
     */
    private String renderNavOption(String path, List<ChenilleApiDefinition> apis) {
        return apis
                .stream()
                .map(api -> {
                    // 查找对应的接口
