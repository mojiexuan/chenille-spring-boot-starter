 ...
 */
@Getter
public enum ChenilleResponseCode {
    ...
    ChenilleResponseCode(String code, HttpStatusCode status, String enMessage, String zhMessage) {
        this.code = code;
        this.status = status;
    ...
    /**
     * 根据 HTTP 状态码获取对应的业务响应码
     */
    public static ChenilleResponseCode getResponseCode(HttpStatusCode status) {
        if (Objects.requireNonNull(status) == HttpStatus.OK) {
            return ChenilleResponseCode.SUCCESS;
        }else if(Objects.requireNonNull(status) == HttpStatus.INTERNAL_SERVER_ERROR) {
            return ChenilleResponseCode.SYSTEM_ERROR;
        }else if(Objects.requireNonNull(status) == HttpStatus.SERVICE_UNAVAILABLE) {
            return ChenilleResponseCode.SERVICE_UNAVAILABLE;
        }else if(Objects.requireNonNull(status) == HttpStatus.BAD_REQUEST) {
            return ChenilleResponseCode.PARAM_ERROR;
        }else if(Objects.requireNonNull(status) == HttpStatus.NOT_FOUND) {
            return ChenilleResponseCode.NOT_FOUND;
        }else {
            return ChenilleResponseCode.SYSTEM_ERROR;
        }
    }
